...
wss.on("connection", async (ws, req) => {
    ...
    console.log(
        `[WS] Connected  connId=${connectionId} userId=${userId} session=${sessionId}`,
    );
    ...
    ws.on("close", async () => {
        ...
        await db
            .query("DELETE FROM ws.connections WHERE connection_id = $1", [
                connectionId,
            ])
            .catch(() => {});
    ...
    ws.on("error", (err) =>
        console.error(`[WS] Error connId=${connectionId}:`, err.message),
    );
});
...
server.listen(WS_PORT, () => {
    ...
    console.log(
        `  Push-back : POST http://localhost:${WS_PORT}/connections/:connectionId`,
    );
});

ensureRmq().catch((err) =>
    console.warn("[RMQ] Initial connect failed, will retry:", err.message),
);
