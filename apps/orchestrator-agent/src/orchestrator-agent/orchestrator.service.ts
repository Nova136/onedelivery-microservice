...
export class OrchestratorService {
    ...
    async processChat(
        ...
        if (!validationResult.isValid || isSuspicious) {
            ...
            const reason = isSuspicious ? "Prompt Injection Detected" : validationResult.error;
            this.logger.warn(`Security threat detected from user ${userId}: [${reason}] ${redactedMessage}`);
        ...
        this.logger.log(`Final Response for session ${session.id}: ${lastAIMessage.content}`);
    ...
    async processAgentCallback(
        ...
        // 1. Get current state for context
        const state = await this.getSessionState(sessionId);
        
        // 2. Run the Agent Callback Graph
        ...
        if (!result.is_safe) {
            this.logger.warn(`Agent update for session ${sessionId} failed safety check. Falling back to safe message.`);
            result.synthesized_message = "We received an update regarding your request, but our system is currently verifying the details. Please check back shortly.";
        }

        this.logger.log(`Final Callback Response for session ${sessionId}: ${result.synthesized_message}`);
