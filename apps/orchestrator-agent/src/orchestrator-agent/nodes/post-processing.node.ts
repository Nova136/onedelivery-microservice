  ...
  return async (state: OrchestratorStateType) => {
    ...
    if (partials.length === 0) {
      ...
      if (!(lastMessage instanceof AIMessage && lastMessage.content)) {
        updatedMessages.push(new AIMessage("I'm sorry, I'm not sure how to help with that. Could you please rephrase your request?"));
      }
      ...
      const userQuery = [...updatedMessages].reverse().find(m => m instanceof HumanMessage)?.content as string || "Unknown query";
      const partialsText = partials.map((p, i) => `Response ${i + 1}:\n${p}`).join("\n\n---\n\n");
      
      const prompt = AGGREGATOR_PROMPT
      ...
      try {
        ...
        logger.error("All models failed for PostProcessing aggregation:", e);
    ...
    if (updatedMessages.length > 6) {
      try {
        newSummary = await summarizer.summarize(updatedMessages.slice(0, -4), state.summary);
