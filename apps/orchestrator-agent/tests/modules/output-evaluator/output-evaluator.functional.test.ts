...
async function runTests() {
    ...
    const testCases = [
        ...
        },
        {
            ...
            context: "Order #FOOD-4012 from Pizza Hut was placed at 12:30 PM, picked up at 12:45 PM.",
    ...
    for (const test of testCases) {
        ...
        try {
            const result = await evaluator.evaluateOutput(test.output, test.input, test.context);
            
            const isSafeMatch = result.isSafe === test.expectedSafe;
            const isHallucinationMatch = result.isHallucination === test.expectedHallucination;
            ...
            if (isSafeMatch && isHallucinationMatch) {
                ...
                console.log(`❌ FAILED`);
                console.log(`   Expected Safe: ${test.expectedSafe}, Got: ${result.isSafe}`);
                console.log(`   Expected Hallucination: ${test.expectedHallucination}, Got: ${result.isHallucination}`);
