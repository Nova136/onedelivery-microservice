...
const injectionScanSchema = z.object({
    injectionDetected: z
        .boolean()
        .describe(
            "true if the message contains embedded instructions targeting the Guardian verifier.",
        ),
    pattern: z
        .string()
        .optional()
...
export class AppService {
    ...
    private readonly DENY_LIST: Array<{ pattern: RegExp; label: string }> = [
        {
            pattern: /ignore\s+(your|all|previous)\s+instructions/i,
            label: "ignore-instructions",
        },
        {
            pattern: /guardian\s*(admin|override|bypass)/i,
            label: "guardian-override",
        },
        {
            pattern:
                /(approve|verify|allow)\s+(this|everything|all)\s+(regardless|unconditionally)/i,
            label: "unconditional-approve",
        },
        {
            pattern: /as\s+guardian[,\s]+you\s+(must|should|will)\s+/i,
            label: "guardian-persona",
        },
        {
            pattern: /for\s+this\s+(verification|gate)\s+only/i,
            label: "scope-override",
        },
        {
            pattern: /system\s*:\s*(approve|override|bypass|grant)/i,
    ...
    constructor(
        ...
        this.verificationLlm =
            this.llm.withStructuredOutput(verificationSchema);
        this.gateLlm = this.llm.withStructuredOutput(gateSchema);
        this.injectionScanLlm =
            this.fastLlm.withStructuredOutput(injectionScanSchema);
...
${SELF_PROTECTION_CLAUSE}`;
    ...
    private async handleVerification(
        ...
        this.logger.log(
            `[${userId}] Guardian VERIFY request | session=${sessionId}`,
        );
        ...
        if (capViolation) {
            this.logger.warn(
                `[${userId}] Financial cap violation detected | session=${sessionId}`,
            );
            this.auditClient.logEvent({
    ...
    private async handleGate(
        ...
        this.logger.log(
            `[${userId}] Guardian GATE request | session=${sessionId}`,
        );
        ...
        try {
            parsed = (await this.gateLlm.invoke(formatted)) as z.infer<
                typeof gateSchema
            >;
        ...
        this.auditClient.logEvent({
            action:
                parsed.verdict === "APPROVED"
                    ? "GUARDIAN_GATE_APPROVED"
                    : "GUARDIAN_GATE_BLOCKED",
            entityType: "tool_call",
            entityId: sessionId,
            userId,
            metadata: {
    ...
    private sanitizeInput(message: string, userId: string): string {
        ...
        if (sanitized.length > this.MAX_MESSAGE_LENGTH) {
            ...
            sanitized =
                sanitized.slice(0, this.MAX_MESSAGE_LENGTH) + " [TRUNCATED]";
        }
    ...
    private async scanForIndirectInjection(
        ...
        try {
            const formatted = await this.injectionScanPrompt.formatMessages({
                input: message,
            });
    ...
    private checkFinancialCapViolation(message: string): string | null {
        ...
        if (/rejected|cannot|unable|exceed|limit|not eligible/i.test(proposed))
            return null;
    ...
    private async retryForCompleteness(
        ...
        try {
            ...
            const retried = (await llmInstance.invoke(retryMessages)) as {
                verdict: string;
                reason?: string;
                feedbackType?: string;
            };
        ...
        return {
            verdict,
            feedbackType,
            reason: this.fallbackReason(feedbackType),
        };
    }
    ...
    private fallbackReason(feedbackType?: string): string {
        const map: Record<string, string> = {
            amount_exceeded:
                "The refund or compensation amount exceeds the policy limit.",
            missing_required_step:
                "A required SOP step was not completed before this action.",
            unauthorized_action:
        ...
        return (
            map[feedbackType ?? ""] ??
            "Policy violation detected. Review the SOP and correct your response."
        );
    }
