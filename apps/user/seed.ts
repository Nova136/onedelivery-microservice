...
const options: DataSourceOptions & SeederOptions = {
  ...
  seeds: [
    (__dirname + '/**/user-record.seed{.ts,.js}').replace(/\\/g, '/'),
...
dataSource.initialize().then(async () => {
  ...
  await setIdNextVal(dataSource, 'user', 'user_record');
  await setIdNextVal(dataSource, 'user', 'historical_user_activity');
  await setIdNextVal(dataSource, 'user', 'user_activity_logging_page_mapping');
  await setIdNextVal(dataSource, 'user', 'user_activity_logging');
  process.exit();
});
