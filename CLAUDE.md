...
- `SemanticRouterService` — classifies intent (`ACTION`, `FAQ`, `ESCALATE`, `END_SESSION`, `UNKNOWN`) using gpt-5.4-mini
- `McpToolRegistryService` — manages available LangChain tools (registered in `onModuleInit`)
- `SpecializedAgentsService` — invokes the action agent (GPT-4o) or FAQ agent (gpt-5.4-mini) depending on routed intent
