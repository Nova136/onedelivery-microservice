...
**Team: NCS Team 16**
_Chee Lim Peng · Yeoh Yong Shan · Chia Kai Xiang · Tan Yee Yuan · Chia Yi Hang_
...
- **AI Agent Layer** — Core: Action Agent (GPT-4o), FAQ Agent (gpt-5.4-mini); Specialist: Logistics, Resolution, Guardian, QA
- **Domain Services** — 7 NestJS services, TypeORM, per-service schema
- **Data** — RabbitMQ event bus + PostgreSQL 17 + pgvector
- **External AI** — OpenAI GPT-4o / gpt-5.4-mini, LangSmith tracing
...
| AI/LLM              | LangChain + LangGraph, GPT-4o, gpt-5.4-mini    |
...
- Reasoning: gpt-5.4-mini (temperature 0); SOP workflow injected from Knowledge service (`PROCESS_REFUND_LOGIC`); 2-hour delivery window enforced, refund math computed per line item (quantity × unit price)
...
- Reasoning: Dual LLM — `gpt-4o` (temperature 0) for gate/verify decisions; `gpt-5.4-mini` for indirect prompt injection scanning. 8 layered guardrails including deny-list regex, hardcoded $20 cap, session compliance budget (1 refund/cancellation per session)
...
- Reasoning: `gpt-5.4-mini` (temperature 0) reviews completed chat sessions to detect service failures, score customer sentiment, and surface incident trends
...
- 5 AI agents (Orchestrator, Logistics, Resolution, Guardian, QA) backed by GPT-4o / gpt-5.4-mini with SOP-grounded pgvector retrieval — reduces hallucination
