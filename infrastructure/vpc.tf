...
# Internet gateway attached to the existing VPC so the public subnets can
# send and receive internet traffic (required for the ALB and for Fargate tasks
# to pull ECR images without a NAT gateway).
resource "aws_internet_gateway" "main" {
  vpc_id = data.aws_vpc.main.id
  tags   = { Name = "${local.name}-igw" }
}

# Dedicated route table for the public subnets — one default route to the IGW.
# New subnets are not automatically associated with any route table, so we
# create one explicitly and bind all three public subnets to it.
